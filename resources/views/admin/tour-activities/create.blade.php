...
<div class="container mx-auto p-4 max-w-4xl">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Create Tour Activity</h1>
        <a href="{{ route('admin.tour-activities.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">
            Back to List
        </a>
    </div>

    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('admin.tour-activities.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="file" name="photo" accept="image/*" 
                           ...
                           required>
                    <p class="text-sm text-gray-500 mt-1">Max 2MB (JPEG, PNG, JPG, WEBP)</p>
            ...
            <div class="mb-6">
                ...
                <div id="bring-container">
                    @if(old('what_to_bring'))
                        @foreach(old('what_to_bring') as $index => $item)
                            <div class="flex gap-2 mb-2 bring-item">
                                <input type="text" name="what_to_bring[]" value="{{ $item }}" 
                                       class="flex-1 p-3 border border-gray-300 rounded-lg" 
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" rows="3" 
                          @class(['w-full p-3 rounded-lg', 'border-red-500 border-2' => $errors->has('notes'), 'border border-gray-300' => !$errors->has('notes')]) 
                          placeholder="e.g., Not suitable for people with heart conditions">{{ old('notes') }}</textarea>
                @error('notes')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="flex gap-3">
                <button type="submit" class="px-6 py-3 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 font-semibold">
                    Create Activity
                </button>
