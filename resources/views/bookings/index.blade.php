...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <h2 class="text-2xl font-bold mb-6">{{ __('messages.my_bookings') }}</h2>

                @if($bookings->isEmpty())
                    <div class="text-center py-8">
                        <i class="fas fa-calendar-alt text-4xl text-gray-400 mb-4"></i>
                        <p class="text-gray-500">{{ __('messages.no_bookings_yet') }}</p>
                        ...
                        @foreach($bookings as $booking)
                            <div class="bg-gray-50 p-6 rounded-lg">
                                ...
                                <div class="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
                                    <div>
                                        <span class="text-gray-500 text-sm">{{ __('messages.date') }}</span>
                                        <p class="font-medium">{{ $booking->date->format('M d, Y') }}</p>
                                    </div>
                                    <div>
                                        <span class="text-gray-500 text-sm">{{ __('messages.guests') }}</span>
