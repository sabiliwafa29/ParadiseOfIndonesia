...
@section('content')
<!-- Hero Section -->
<div class="bg-gradient-to-br from-orange-600 via-red-600 to-pink-600 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 relative">
        <div class="text-center">
            <div class="inline-block px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full border border-white/30 mb-4">
                <span class="text-white font-medium text-sm md:text-base">✨ {{ __('messages.destination_highlights') ?? 'Destination Highlights' }}</span>
            </div>
            <h1 class="text-3xl md:text-4xl lg:text-5xl font-extrabold text-white mb-4">
                {{ __('messages.destination_highlights') ?? 'Destination Highlights' }}
            </h1>
            <p class="text-lg md:text-xl text-white/90 max-w-2xl mx-auto">
                {{ __('messages.featured_activities') ?? 'Featured activities & attractions' }}
            </p>
        </div>
    </div>
</div>
            ...
            @forelse($activities as $activity)
                <div class="group bg-white rounded-3xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                    ...
                    <div class="p-4 md:p-6">
                        ...
                        <!-- Time Info -->
                        <div class="flex items-center text-gray-500 text-sm md:text-base mb-4">
                ...
                <div class="col-span-full text-center py-12">
                    <div class="inline-block p-6 bg-gray-50 rounded-2xl">
                        ...
                        <p class="text-sm md:text-base text-gray-500">
                            {{ __('messages.no_activities_found') ?? 'No tour activities found.' }}
                        </p>
                    </div>
                </div>
            @endforelse
